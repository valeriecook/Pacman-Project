...
class AlphaBetaAgent(MultiAgentSearchAgent):
  ...
  def max_prune(self, gameState, depth, agentIndex, alpha, beta):
      ...
      for action in gameState.getLegalActions(0):
          ...
          temp = self.min_prune(successor, depth, 1, alpha, beta)
      ...
      for action in gameState.getLegalActions(agentIndex):
          ...
          if agentIndex == numAgents - 1:
              if depth == self.depth:
                  ...
                  temp = self.max_prune(successor, depth+1, 0, alpha, beta)
          else:
              temp = self.min_prune(successor, depth, agentIndex+1, alpha, beta)
...
def betterEvaluationFunction(currentGameState):
    """
      ...
      DESCRIPTION: I have subtracted 10 points for every pellet of food left in the game
      20 points are added for each step away the nearest ghost is in terms of maze distance
      Scared ghosts are valued at 200 minus the distance they are from the pacman
      The base value is the current score of the game
    """
    ...
    for ghost in currentGameState.getGhostStates():
        ...
        if ghost.scaredTimer == 0:
            if manhattanDistance(currentGameState.getPacmanPosition(), ghostPosition) < nearestGhostDistance:
                nearestGhostDistance = manhattanDistance(currentGameState.getPacmanPosition(), ghostPosition)
    
        elif ghost.scaredTimer > manhattanDistance(currentGameState.getPacmanPosition(), ghostPosition):
            ghostEval += 200 - manhattanDistance(currentGameState.getPacmanPosition(), ghostPosition)
    ...
    return currentGameState.getScore() - 10*currentGameState.getNumFood() + 1*ghostEval 
